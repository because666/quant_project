...
def build_dmats(
    *,
    ...
    y_tr_rel = return_aware_relevance(y_tr, g_tr)
    y_va_rel = return_aware_relevance(y_va, g_va)
...
def build_dmats_with_label_fn(
    ...
    *,
    ...
    y_va_rel = label_fn(y_va, g_va, **kw)
...
def train_final_xgboost_with_label_fn(
    ...
    *,
    ...
    参数:
        ...
        importance_path: 特征重要性保存路径，为 None 时使用 models/{label_fn_name}_xgboost_feature_importance.json
        log_path: 训练日志路径，为 None 时使用 models/{label_fn_name}_xgboost_training.log
...
def train_final_xgboost_e1a(
    *,
    ...
    确保标签函数与 metrics.json 中记录的 label_fn_name 一致。

    模型默认保存为 models/xgboost.json（与 train_final_xgboost 一致），
    指标保存为 models/xgboost_metrics.json（额外包含 label_fn_name 和 train_val_gap 字段）。
    ...
    if importance_path is None:
        importance_path = MODELS_DIR / "xgboost_feature_importance.json"
    if log_path is None:
        log_path = MODELS_DIR / "xgboost_training.log"
    ...
    if tune:
        logger.info(
            "超参数搜索：%d 次 trial，目标=验证集 NDCG@10，early_stopping=%d",
